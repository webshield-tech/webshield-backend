...
function buildWhoisReportContent(scan, language = 'english') {
  const res = scan.results?.whois || scan.results || {};
  const whoisData = String(res.data || res.rawOutput || '').slice(0, 2000) || 'No WHOIS data available.';

  const content = `
VULN SPECTRA SECURITY SCAN REPORT (WHOIS)

Scan Overview
----------------------
Scan ID       : ${scan._id}
Target        : ${scan.targetUrl}
Type          : WHOIS (Informational)
Date          : ${new Date(scan.createdAt).toLocaleString()}
Language      : ${language}

Informational WHOIS Data
----------------------
${whoisData}

Notes
----------------------
WHOIS is an informational lookup. It does not indicate vulnerabilities.
...
async function ensureReportForLanguage(scan, language) {
  ...
  try {
    if (scan.scanType === 'whois') {
      scan.reportContent = buildWhoisReportContent(scan, requestedLanguage);
...
function buildBatchReportTextContent({ batchId, targetUrl, scans, analysisJson, language = 'english' }) {
  ...
  const whoisScan = scans.find((s) => String(s.scanType).toLowerCase() === 'whois');
  const whoisData = whoisScan?.results?.whois?.data || whoisScan?.results?.rawOutput || '';
  const whoisText = whoisScan
    ? `\nInformational WHOIS Lookup\n----------------------\n${String(whoisData).slice(0, 1200) || 'WHOIS data not available.'}\n`
    : '';
